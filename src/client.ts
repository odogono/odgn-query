...
export class QueryClient {
  ...
  private emitEvent(type: EventType, key?: QueryKey, meta?: unknown) {
    ...
    this.emitter.emit(type, ev as any);
    this.emitter.emit('event', ev as any);
  }
  ...
  async setQueryData<T>(key: QueryKey, value: T, ttl?: number): Promise<void> {
    await this.ensureReady();
    if (this.cache.set) {
      await this.cache.set<T>(key, value, ttl ?? this.defaults.ttl);
    } else {
      // Fallback via wrap to persist value
      await this.cache.wrap<T>(
        key,
        () => value,
...
export type EventMap = {
  CLEAR_ALL: BaseEvent;
  ERROR: BaseEvent;
  event: BaseEvent; // aggregated channel for backwards compatibility
  FETCH: BaseEvent;
  HIT: BaseEvent;
  INVALIDATE: BaseEvent;
  INVALIDATE_BRANCH: BaseEvent;
  MISS: BaseEvent;
  MUTATION_COMPLETE: BaseEvent;
  MUTATION_ERROR: BaseEvent;
  MUTATION_START: BaseEvent;
  'STALE→SWR': BaseEvent;
};
